...
<template><section class="search-panel"><label for="site-search">{{ props.lang === 'en' ? 'Search articles' : '搜索文章' }}</label><Input id="site-search" v-model="query" :placeholder="props.lang === 'en' ? 'Search titles, content, categories, or tags' : '输入标题、正文、分类或标签'" /><p v-if="!ready" class="search-note">{{ props.lang === 'en' ? 'The search index is available after a production build.' : '搜索索引仅在生产构建后可用。' }}</p><p v-else-if="searchedTerms.length" class="search-terms">{{ props.lang === 'en' ? 'Expanded terms:' : '已扩展检索：' }}<span v-for="term in searchedTerms" :key="term">{{ term }}</span></p><Spin v-if="loading" /><div v-else-if="results.length" class="search-results" aria-live="polite"><a v-for="result in results" :key="result.url" :href="result.url"><h2>{{ result.meta.title }}</h2><p v-html="result.excerpt"></p><small v-if="result.matchedTerms?.length">{{ props.lang === 'en' ? 'Matched:' : '命中：' }} {{ result.matchedTerms.join(' · ') }}</small></a></div><p v-else-if="query && ready" class="search-note">{{ props.lang === 'en' ? 'No matching articles found.' : '没有找到匹配文章。' }}</p></section></template>

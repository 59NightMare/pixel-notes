...
- Primary navigation: 中文为首页、归档、分类、标签、关于；英文为 Home、Archive、Categories、Tags、About；右侧为语言、搜索与主题切换。
...
- 内容入口保持同一种交互语法：Hero 主次操作、首页主题索引与分类/标签筛选均使用单一链接控件，统一硬边框、功能色阴影、按压位移和当前状态，禁止链接嵌套按钮。
- 文章卡片按阅读任务分级：精选卡负责建立视觉记忆，保留大图但让标题和阅读入口可见；归档、分类和标签使用紧凑横向卡，优先展示标题、摘要和元信息，避免封面占据整个首屏。
- 文章详情保持一套阅读框架：中英文共用标题层级、元数据、主题标签、封面图注、桌面/移动目录、作者信息、前后篇和讨论状态；语言只改变文案与链接目标，不改变页面能力。
- 搜索是一项检索任务：输入区保持单一紧凑工具面板，结果使用编号索引行并明确显示查询词与数量；空白、校验、加载、无结果和索引不可用均有独立状态，中英文能力完全一致。
- 作者身份是信任入口：“关于”页使用身份事实、作者叙事与结构化联系行建立层级，不把每段内容塞进同权重卡片。
...
- Motion: 120-180ms 的整数像素反馈；普通导航悬停仅上移 1px 并显示黄色短底线，不铺大面积背景；工具按钮悬停提高对应功能色浓度并加深硬阴影；搜索和主题图标可进行轻微缩放/旋转；不循环漂浮；减少动态效果时全部停用。
...
- New/changed components: `BrandMark`、`HeaderActions`、语言切换、`SearchPanel` 检索台与编号结果行、`NewsletterForm`、页脚品牌导航、Hero 操作链接、主题索引、分类/标签内容地图、精选文章卡、紧凑文章卡、作者档案与联系行、`Comments`、PWA 注册、离线页和 CMS 管理入口；沿用现有 Pixelium 控件与硬边框语言。
- Variants and states: 头部与页脚共用像素笔记页 Logo 与 `PIXEL / NOTES` 锁定；页脚保持单一绿色边界、无嵌套卡片，订阅不可用时以 RSS 正向入口替代开发态警告；Hero 操作使用主/次链接，侧栏使用紧凑索引行，分类与标签使用带编号和计数的选择链接，黄色表示当前项；“关于”页以无框作者叙事为主、联系区以左侧功能色边界分栏，联系行具有整行点击、方向箭头、悬停内缩高亮与按压反馈；Logo 图形与右侧工具按钮共用“低饱和底色 + 功能色硬阴影 + 悬停位移”语法，品牌文字保持扁平；导航默认/无底色悬停/黄色当前页/聚焦/按下；语言、搜索、主题保持独立方形工具按钮，默认使用与头部同源的低饱和功能色填充，并分别使用珊瑚红、天蓝、紫色硬阴影和悬停位移；外部服务增加未配置、加载、成功和服务端错误状态。
...
- Layout adaptations: 首页双栏变单栏；普通文章卡在桌面端使用横向媒体与内容布局，移动端折叠为上下结构；中英文文章均在桌面端固定右侧目录，移动端折叠为目录按钮；导航变抽屉；400px 以下顶部仅保留搜索和菜单，语言与主题转入菜单。
...
- Search: 单字符查询在本地提示并阻止检索；检索中维持固定高度；结果区反馈当前查询和数量；摘要只包含标题、摘要、标签与正文，不混入日期栏、目录、面包屑和前后篇导航。
